...
from models.eegpt_probe_head import EEGPTLinearProbeHead
...
class DownstreamEEGVAE(nn.Module):
    ...
    def __init__(
        ...
        head_type = str(head_config.pop("type", "default")).lower()
        if head_type in {"eegvae_probe", "vae_probe", "latent_probe"}:
            ...
            self.classifier = EEGPTLinearProbeHead(
                task=task,
                n_classes=n_classes,
                output_dim=output_dim,
                **head_config,
        ...
        if checkpoint_path is not None and self.mode != "full_training":
            self._load_encoder(checkpoint_path, strict=strict)
    ...
    def _load_encoder(self, checkpoint_path: str, strict: bool = False):
